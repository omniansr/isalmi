...
class QuranService {
 ...
 static List<Sura> suras= List.generate(114, (index) =>
 getSuraFromIndex(index));
 static List<Sura> mostRecentlySuras =[];
 
 static Sura getSuraFromIndex(index) => Sura
   (englishName: englishSuraNames[index],
     arabicName: arabicSuraNames[index], 
     ayatCount: ayaNumbers[index],
     num: index + 1,);
 ...
 static Future<void> addSuraToMostRecently(Sura sura) async {
     mostRecentlySuras.removeWhere((s) => s.num == sura.num);
     mostRecentlySuras.add(sura);

    List<String> mostRecentlyIndexes =
   mostRecentlySuras.map((sura) => (sura.num - 1).toString()).toList();
    final sharedPref = await SharedPreferences.getInstance();
   await sharedPref.setStringList('mostRecentlyIndexes', mostRecentlyIndexes);
